...
import MoreButton from 'components/userGroup/GroupMoreButton';
import DlgGroupDetail from 'component/dialog/GroupDetailAdmin';
import GroupStatusChange from 'component/dialog/GroupStatusChange';
...
export default function GroupTable(props) {
	...
	const showToggleChange = (data) => {
		setShowChange(data);
		showDialog(dialogList.GROUP_STATUS_CHANGE);
	};
	const showToggleDetail = (data) => {
		setShowDetail(data);
		showDialog(dialogList.GROUP_DETAIL_ADMIN);
	};

	return (
		<>
			<DlgGroupDetail data={showDetail} />
			<GroupStatusChange data={showChange} fetchList={fetchList} />
			<StyledTable>
						...
						>
							{
								<>
									...
									<td>
										<TD_5 style={{ color: colors.light_blue }}>{data.end_date}</TD_5>
									</td>
									<td>
										<TD_6>
											<MoreButton data={data} showToggleDetail={showToggleDetail} showToggleChange={showToggleChange} />
										</TD_6>
									</td>
								</>
							}
